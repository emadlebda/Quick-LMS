    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.questions-options.update", [$questionsOption->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('question') ? 'is-invalid' : '' }}" name="question_id" id="question_id">
                    @foreach($questions as $id => $question)
                        <option value="{{ $id }}" {{ (old('question_id') ? old('question_id') : $questionsOption->question->id ?? '') == $id ? 'selected' : '' }}>{{ $question }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('option_text') ? 'is-invalid' : '' }}" name="option_text" id="option_text" required>{{ old('option_text', $questionsOption->option_text) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_correct') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="is_correct" id="is_correct" value="1" {{ $questionsOption->is_correct || old('is_correct', 0) === 1 ? 'checked' : '' }} required>
