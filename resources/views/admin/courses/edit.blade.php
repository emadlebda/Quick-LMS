...
{{--            @if(auth()->user()->is_admin)--}}
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('teachers') ? 'is-invalid' : '' }}" name="teachers[]" id="teachers" multiple>
                    @foreach($teachers as $id => $teachers)
                        <option value="{{ $id }}" {{ (in_array($id, old('teachers', [])) || $course->teachers->contains($id)) ? 'selected' : '' }}>{{ $teachers }}</option>
                    @endforeach
                </select>
...
{{--            @endif--}}
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $course->slug) }}">
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('course_image') ? 'is-invalid' : '' }}" id="course_image-dropzone">
                </div>
